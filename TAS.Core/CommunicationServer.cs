...
public static class CommunicationServer {
    ...
    public static event Func<string, string> ReturnDataEvent; 
        ...
        server = new TcpServer(port.Value) {
            ...
            OnReceived = c => {
                ...
                switch (serverToClientMessage) {
                    case HotkeyMessage hotkeyMessage:
                        ReceiveHotkeyMessage(hotkeyMessage);
                        break;
                    case PathMessage pathMessage:
                        ReceivePathMessage(pathMessage);
                        break;
                    case GetGameDataMessage getGameDataMessage:
                        ReceiveGetGameDataMessage(getGameDataMessage);
                        break;
                }
            },
    ...
    private static void ReceiveGetGameDataMessage(GetGameDataMessage getGameDataMessage) {
        if (ReturnDataEvent != null) {
            SendMessage(new ReturnGameDataMessage(ReturnDataEvent.Invoke(getGameDataMessage.DataType)));
        } else {
            SendMessage(getGameDataMessage.DataType == "Load"
                ? new ReturnGameDataMessage($"Load {Manager.Game.LevelName}")
                : new ReturnGameDataMessage(null));
        }
    }
}