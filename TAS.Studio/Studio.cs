...
public partial class Studio : BaseForm {
            ...
            if (e.Modifiers == Keys.Control) {
                switch (e.KeyCode) {
                    ...
                    case Keys.Down: // Ctrl + Down
                        GoDownLabelAndBreakpoint(e);
                        break;
                    case Keys.Up: // Ctrl + Up
                        GoUpLabelAndBreakpoint(e);
                        break;
                    case Keys.L: // Ctrl + L
                        CombineInputs(true);
                        break;
    ...
    private void InsertLoadCommand() {
        if (GetDataFromGame("Load") is { } gameData) {
            InsertNewLine(gameData);
        } else {
            InsertNewLine("Load (LevelName)");  
        }
    }

    private string GetDataFromGame(string gameType) {
        CommunicationWrapper.ReturnData = null;
        CommunicationClient.SendMessage(new GetGameDataMessage(gameType));
    ...
    private void GoDownLabelAndBreakpoint(KeyEventArgs e) {
    ...
    private void GoUpLabelAndBreakpoint(KeyEventArgs e) {
        List<int> commentLine = richText.FindLines(@"^\s*@|^\*\*\*");
        if (commentLine.Count > 0) {
    ...
    private void TryGoToPlayCommand() {
        string label = richText.CurrentStartLineText.Trim();
        if (!label.StartsWith("@") || label.Length < 2) {
            return;
        }

        label = label.Substring(1, label.Length - 1);
        Regex labelRegex = new(@$"{label}( |,|$)");
        Regex playRegex = new(@"^#?play", RegexOptions.IgnoreCase);
        int? lineNumber = null;
        for (int i = 0; i < richText.Lines.Count; i++) {
            string line = richText.Lines[i].Trim();
            if (playRegex.IsMatch(line) && labelRegex.IsMatch(line)) {
                lineNumber = i;
                break;
            }
        }
    ...
    private void InsertRoomName() => InsertNewLine($"@{CommunicationWrapper.GameInfo?.LevelName}");
    ...
    private void CommentText(bool toggle) {
        ...
        foreach (InputFrame record in selection) {
            if (record.IsLabel || record.IsCommentTime) {
    ...
    private void homeMenuItem_Click(object sender, EventArgs e) {
        Process.Start("https://github.com/DemoJameson/TheMessenger.TAS");
    }
