...
public class InputFrame : InputFrameBase {
    ...
    private static readonly Regex LabelSymbolRegex = new(@"^\s*@", RegexOptions.Compiled);
    public static readonly Regex LabelLineRegex = new(@"^\s*@.*", RegexOptions.Compiled);
    ...
    public InputFrame(string line) {
        ...
        if (Frames <= 0) {
            if (CommentSymbolRegex.IsMatch(line)) {
                IsComment = true;
                if (CommentTimeRegex.IsMatch(line)) {
                    IsCommentTime = true;
                }
            } else if (LabelSymbolRegex.IsMatch(line)) {
                IsLabel = true;
