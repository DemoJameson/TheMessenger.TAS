...
public static class Drawing {
    ...
    public static void DrawCircle(Vector2 center, int radius, Color color, float width, bool antiAlias, int segmentsPerQuarter) {
        ...
        Vector2 p1_tan_a = new(center.x - rh, center.y - radius);
        Vector2 p1_tan_b = new(center.x + rh, center.y - radius);

        Vector2 p2 = new(center.x + radius, center.y);
        Vector2 p2_tan_a = new(center.x + radius, center.y - rh);
        Vector2 p2_tan_b = new(center.x + radius, center.y + rh);

        Vector2 p3 = new(center.x, center.y + radius);
        Vector2 p3_tan_a = new(center.x - rh, center.y + radius);
        Vector2 p3_tan_b = new(center.x + rh, center.y + radius);

        Vector2 p4 = new(center.x - radius, center.y);
        Vector2 p4_tan_a = new(center.x - radius, center.y - rh);
        Vector2 p4_tan_b = new(center.x - radius, center.y + rh);
    ...
    private static void Initialize() {
        ...
        // depending on the alphaBlend parameter.
